...
1. Integrals over $p_0$ replaced by sums over $\omega_n$
...
1. **Vacuum contribution** (the "1" in $\coth = 1 + 2n_B$)
2. **Thermal contribution** (the $n_B$ or $n_F$ pieces)
...
3. Use propagator structure with $p_0 \to i\omega_n$
...
$$n_B(E) = \frac{1}{e^{\beta E} - 1}$$

**Fermi-Dirac distribution:**
$$n_F(E) = \frac{1}{e^{\beta E} + 1}$$

**Key sum identities:**
$$T\sum_n\frac{1}{\omega_n^2 + E^2} = \frac{1}{2E}\coth(\beta E/2) \text{ (bosons)} $$

$$T\sum_n\frac{1}{\omega_n^2 + E^2} = \frac{1}{2E}\tanh(\beta E/2) \text{ (fermions)}$$

**Decomposition:**
$$\coth(\beta E/2) = 1 + 2n_B(E)$$
$$\tanh(\beta E/2) = 1 - 2n_F(E)$$
...
2. Compute the thermal scalar mass from the tadpole diagram at one loop, and verify $m^2_{\rm th} = \lambda T^2/24$.

3. Using the high-temperature expansion of the effective potential, derive the critical temperature $T_c^2 = 24m^2/\lambda$ for $\phi^4$ theory with $m^2 > 0$ (symmetry restoration).
